package cn.wzzcd6.gateway.filter;
...
import cn.wzzcd6.common.constant.TokenConstant;
import cn.wzzcd6.common.utils.JsonUtil;
...
public class AuthorizeFilter implements GlobalFilter, Ordered {
	...
	// 排除过滤的 uri 地址
	private static final String[] ignoreUrls = { "/user/login", "/user/register", };
...
//        Result result = new ResultUtil<>().setErrorMsg(HttpStatus.UNAUTHORIZED.value(), HttpStatus.UNAUTHORIZED.getReasonPhrase());
		Result resp = new Result(HttpStatus.UNAUTHORIZED.value(), HttpStatus.UNAUTHORIZED.getReasonPhrase());
